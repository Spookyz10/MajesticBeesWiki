async function loadCodes() {
    ...
    expiredTarget.innerHTML = renderCodeList(expiredCodes, true);
...
function isExpired(code) {
    if (!code.expires || code.expires === "Unknown") {
        return true;
    }

    const expiryTime = parseDate(code.expires);
...
function renderRewardGroup(label, items) {
    ...
    const body = items.map(item => `<div class="code-reward-item">${escapeHtml(item)}</div>`).join("");
...
function renderCard(code, expired) {
    const limitedUses = code.limited_uses ? `<div class="code-uses">Limited: ${escapeHtml(code.limited_uses)} uses</div>` : "";
    const expiryLabel = expired
        ? (code.expires && code.expires !== "Unknown" ? `Expired ${escapeHtml(code.expires)}` : "Expired - date unknown")
    ...
    const rewards = rewardGroups || `<div class="code-no-rewards">No reward information available.</div>`;
    ...
    return `
        <div class="code-card${expired ? " code-card-expired" : ""}">
            <div class="code-card-top">
                ...
                <div class="code-expiry${expired ? " code-expiry-expired" : ""}">${expiryLabel}</div>
            </div>
